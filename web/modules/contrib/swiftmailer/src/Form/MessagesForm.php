...
class MessagesForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    // The available text formats are restricted to ones that to convert to plain text or at least use filtered html.
    foreach (filter_formats($this->currentUser()) as $format) {
      if ($format->filters('filter_html_escape')->status || $format->filters('filter_html')->status) {
    ...
    $form['html_convert']['format'] = [
      ...
      '#description' => $this->t('The list of available formats is restricted to those that use escaped or filtered HTML.'),
    ];
