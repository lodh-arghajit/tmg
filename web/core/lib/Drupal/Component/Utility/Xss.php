...
class Xss {
      ...
      switch ($mode) {
        case 0:
          ...
          if (preg_match('/^([-a-zA-Z][-a-zA-Z0-9]*)/', $attributes, $match)) {
            ...
            $skip = (substr($attribute_name, 0, 2) == 'on');
        ...
        case 2:
          ...
          if (preg_match('/^"([^"]*)"(\s+|$)/', $attributes, $match)) {
            $value = static::filterAttributeValue($attribute_name, $match[1]);
  ...
  /**
   * Filter an attribute value for XSS.
   *
   * Given an attribute name and value, filter the value for XSS. Return the
   * cleaned value or FALSE if safety cannot be determined and the entire
   * attribute should be dropped.
   *
   * Child classes can override this function to add additional value checks
   * or allow some values of 'style'.
   *
   * @param string $attribute_name
   *   The attribute name.
   * @param string $attribute_value
   *   The attribute value to filter and return.
   *
   * @return mixed
   *   The filtered string or FALSE if the value cannot be filtered.
   */
  protected static function filterAttributeValue($attribute_name, $attribute_value) {
    if (substr($attribute_name, 0, 2) == 'on') {
      return FALSE;
    }
    elseif ($attribute_name == 'style') {
      return static::filterStyleAttributeValue($attribute_value);
    }
    elseif (static::needsProtocolFiltering($attribute_name)) {
  ...
  /**
   * Determine if an attribute should be filtered for malicious protocols.
   *
   ...
   *
   * @return bool
   *   TRUE if the attribute needs protocol filtering.
   *
   * @see \Drupal\Component\Utility\UrlHelper::filterBadProtocol()
   * @see http://www.w3.org/TR/html4/index/attributes.html
   */
  protected static function needsProtocolFiltering($attribute_name) {
    return (substr($attribute_name, 0, 5) !== 'data-' && !in_array($attribute_name, [
      'title',
      'alt',
      'rel',
      'property',
  ...
  /**
   * Filter the value of an inline "style" attribute.
   *
   * @param string $attribute_value
   *   The attribute value to filter and return.
   *
   ...
   *   The filtered string or FALSE if the value cannot be filtered.
   */
  protected static function filterStyleAttributeValue($attribute_value) {
    // This implementation allows simple style values like:
    // - width: 50%
    // - color: #ac45ff
    // - background-color: blue
    // - margin-top: 3px
    // - display: flex
    // Any complex transforms or URLs are disallowed by the lack of parenthesis.
    $filtered_styles = [];
    foreach (explode(';', $attribute_value) as $style) {
      // Only consider well-formed property/value pairs.
      if (preg_match('/^(\s*)([a-z-]+)(\s*:\s*)(.+)(\s*)$/i', $style, $m)) {
        $preSpace = $m[1];
        $property = $m[2];
        $delimiter = $m[3];
        $value = $m[4];
        ...
        // Only allow values that are simple letters and numbers spaces, and
        // unit symbols, disallowing parenthesis, urls, and other
        // potentially dangerous values.
        if (preg_match('/^[ a-z0-9_,%#\.-]+$/i', $value)) {
          $filtered_styles[] = $preSpace . $property . $delimiter . $value . $postSpace;
        }
      }
    }
    if (empty($filtered_styles)) {
      ...
      $filtered_style_string = implode(';', $filtered_styles);
      // Include trailing semicolons to maintain original content formatting.
      if (preg_match('/(;\s*)$/', $attribute_value, $m2)) {
        $filtered_style_string .= $m2[1];
      }
      return $filtered_style_string;
    }
  }
