...
(function ($, Drupal) {
  ...
  Drupal.evaluatePasswordStrength = function (password, passwordSettings) {
    ...
    if (password.length < 8) {
      msg.push(Drupal.t("Make it at least 8 characters"));
      strength -= (8 - password.length) * 5 + 30;
    }

    if (!hasLowercase) {
      msg.push(Drupal.t("Add lowercase letters"));
      weaknesses += 1;
    }

    if (!hasUppercase) {
      msg.push(Drupal.t("Add uppercase letters"));
      weaknesses += 1;
    }

    if (!hasNumbers) {
      msg.push(Drupal.t("Add numbers"));
      weaknesses += 1;
    }

    if (!hasSpecial) {
      msg.push(Drupal.t("Add special characters"));
      weaknesses += 1;
    }
